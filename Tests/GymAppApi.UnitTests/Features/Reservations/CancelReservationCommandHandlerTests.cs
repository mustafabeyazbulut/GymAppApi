...
public class CancelReservationCommandHandlerTests
{
    ...
    public async Task Handle_WhenReservationDoesNotExist_ThrowsNotFoundException()
    {
        ...
        var handler = new CancelReservationCommandHandler(uow.Object);

        await Assert.ThrowsAsync<NotFoundException>(() =>
            handler.Handle(new CancelReservationCommand { ReservationId = 1, RequestedByUserId = CallerId }, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WhenCallerIsTheReservationsOwnMember_CancelsIt()
    {
        var reservation = BookedReservation();
        var (uow, writeRepo) = Wire(reservation, callerAssignments: new List<Assignment>());
        var handler = new CancelReservationCommandHandler(uow.Object);

        await handler.Handle(new CancelReservationCommand { ReservationId = 1, RequestedByUserId = MemberId }, CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenCallerIsTheReservationsOwnTrainer_CancelsIt()
    {
        var reservation = BookedReservation();
        var (uow, writeRepo) = Wire(reservation, callerAssignments: new List<Assignment>());
        var handler = new CancelReservationCommandHandler(uow.Object);

        await handler.Handle(new CancelReservationCommand { ReservationId = 1, RequestedByUserId = TrainerId }, CancellationToken.None);
    ...
    public async Task Handle_WhenReservationIsNotBooked_ThrowsReservationNotBookedException()
    {
        ...
        await Assert.ThrowsAsync<ReservationNotBookedException>(() =>
            handler.Handle(new CancelReservationCommand { ReservationId = 1, RequestedByUserId = MemberId }, CancellationToken.None));
        writeRepo.Verify(r => r.Update(It.IsAny<Reservation>()), Times.Never);
    }
}
