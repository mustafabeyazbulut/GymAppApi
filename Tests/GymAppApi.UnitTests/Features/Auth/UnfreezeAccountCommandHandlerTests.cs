...
public class UnfreezeAccountCommandHandlerTests
{
    ...
    public async Task Handle_WhenUserNotFound_ThrowsNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() =>
            handler.Handle(new UnfreezeAccountCommand { UserId = 1, Code = "123456" }, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WithValidCode_ClearsIsAccountFrozen_AndConsumesPending()
    {
        ...
        await handler.Handle(new UnfreezeAccountCommand { UserId = 1, Code = "123456" }, CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WithWrongCode_ThrowsInvalidContactVerificationCodeException_AndDoesNotUnfreeze()
    {
        var user = new User { Id = 1, FullName = "Ayşe", Phone = "+905551112233", PasswordHash = "x", IsAccountFrozen = true };
        var pending = ValidPendingFor(user.Phone);
        ...
        await Assert.ThrowsAsync<InvalidContactVerificationCodeException>(() =>
            handler.Handle(new UnfreezeAccountCommand { UserId = 1, Code = "000000" }, CancellationToken.None));
