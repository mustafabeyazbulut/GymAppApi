...
public class ForgotPasswordCommandHandlerTests
{
    private static (Mock<IUnitOfWork> uow, Mock<IReadRepository<User>> userReadRepo, Mock<IReadRepository<OtpVerification>> otpReadRepo,
        Mock<IWriteRepository<OtpVerification>> otpWriteRepo, Mock<ISmsSender> sms, Mock<IEmailSender> email,
        Mock<IPhoneNumberNormalizer> phoneNormalizer) Wire(
            User? found, IReadOnlyList<OtpVerification>? priorLiveOtps = null)
    {
        ...
        return (uow, userReadRepo, otpReadRepo, otpWriteRepo, sms, email, phoneNormalizer);
    }

    [Fact]
    public async Task Handle_WhenUserNotFound_StillReturnsGenericSuccess_AndSendsNothing()
    {
        var (uow, _, _, otpWriteRepo, sms, email, phoneNormalizer) = Wire(found: null);
    ...
    public async Task Handle_WhenUserHasEmail_SendsEmailNotSms()
    {
        ...
        var (uow, _, _, otpWriteRepo, sms, email, phoneNormalizer) = Wire(user);
        var handler = new ForgotPasswordCommandHandler(uow.Object, sms.Object, email.Object, phoneNormalizer.Object);
    ...
    public async Task Handle_WhenPriorLiveOtpExists_InvalidatesItBeforeIssuingNewOne()
    {
        ...
        var (uow, _, otpReadRepo, otpWriteRepo, sms, email, phoneNormalizer) = Wire(user, new List<OtpVerification> { priorOtp });
        var handler = new ForgotPasswordCommandHandler(uow.Object, sms.Object, email.Object, phoneNormalizer.Object);
