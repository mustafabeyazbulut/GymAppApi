...
public class CreateDoorCommandHandlerTests
{
    ...
    public async Task Handle_WhenZoneDoesNotExist_ThrowsNotFoundException()
    {
        ...
        var handler = new CreateDoorCommandHandler(uow.Object);

        await Assert.ThrowsAsync<NotFoundException>(() =>
            handler.Handle(new CreateDoorCommand { ZoneId = 1, Name = "Kapı 1", RequestedByUserId = CallerId }, CancellationToken.None));
    }

    [Fact]
    public async Task Handle_WhenCallerIsAuthorized_CreatesDoor()
    {
        ...
        var result = await handler.Handle(new CreateDoorCommand { ZoneId = 1, Name = "Kapı 1", RequestedByUserId = CallerId }, CancellationToken.None);

        Assert.Equal("Kapı 1", result.Name);
    }
