...
1. **Business rules fix (small, backend-only)** — enforce the GymAdmin+BranchManager same-company block above. Not yet built.
2. **Package core module** — `Package` (catalog/template: name, description, scope, duration/session-count, an extensible AccessTier-style field for future content-gating, IsActive) + `PackageAssignment` (Member ← Package, dates, status). Assignment requires the **same SMS-confirmation flow as staff invitations** ([[project-assignment-invitation-security]]'s pattern) — user explicitly chose this over direct/instant assignment. Only **GymAdmin/BranchManager** may give a package (Trainer explicitly excluded, unlike the earlier draft's open question). Not yet built.
...
**Current status: nothing in this list is built yet.** This session is mid-brainstorm on step 2 (Package core module) when this file was last updated. Check `docs/superpowers/plans/` for a `*-package-*` plan file before assuming otherwise — if the plan for a given step exists and shows tasks checked off, trust that over this summary.
