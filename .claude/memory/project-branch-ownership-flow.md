...
## Mobile gap found and closed (2026-09-19, overnight unsupervised session)

Discovered by auditing "which backend Commands have zero mobile repository method calling them" while working through [[project-member-package-linkage-design]]'s Phase-1-completion pass: **`POST /api/branches` (and PATCH .../active, PATCH /{id}) had NO mobile UI at all**, despite this file's own "New flow end to end" section above describing step 3 (GymAdmin creates their own branch) as a required, non-optional step. A new GymAdmin who confirmed their invitation via `ConfirmInvitationScreen` had no way to create a branch through the app - the entire onboarding flow was a dead end, worse than the Package-management gap since without a branch nothing else (staffing, packages scoped to a branch) can happen either. This was arguably the single most severe gap found all session.

Fixed with a new GymApp `BranchManagementScreen` ("Şubelerim" in the drawer, right above "Personel Ekle" - ordering matters, branch comes first): list + create + rename/re-address (shared `branch_form_sheet.dart` bottom sheet) + deactivate-with-confirmation (the confirm dialog explicitly warns that a closed branch disappears and only platform management can reopen it, since that's a real, user-facing consequence of this file's Task 6 limitation, not just an internal implementation detail). Visible to both GymAdmin and BranchManager (`staffAssignment != null`, same gating as every other staff drawer entry), but create/rename/deactivate controls only render when `role == 'GymAdmin'` - matches backend's `GymAdminOrSuperAdmin` policy on Create/Update/SetActive exactly (BranchManager gets a read-only view of their own branch).
