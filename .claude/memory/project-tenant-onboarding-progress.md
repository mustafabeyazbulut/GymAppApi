...
- [x] Task 11 — Retire self-service registration — **EXECUTED THEN REVERTED, WRONG.** Executed as commit `345c6f5` (2026-09-17), following the plan text literally. The user then corrected the actual product model: registration is permanent, companies assign already-registered users rather than creating new accounts by phone. Reverted as commit `967fc43`. **Do not redo this task.** Full incident in [[feedback-never-remove-registration-pointer]] (full detail in GymApp's own memory).

## Plan status (2026-09-17): Tasks 1-10 done. Task 11 reverted — see above, do not redo. Also grew a small unplanned addition along the way: `GET/PATCH /api/companies`, `/api/companies/{id}`, `/api/companies/{id}/active` (list/detail/rename/activate-deactivate, commit `b74bcfe`) — requested by the user mid-mobile-plan to back a "Firma Yönetimi" management screen instead of a bare create-company form. See GymApp's own `project-tenant-onboarding-mobile-status.md` for the full story.

**Real remaining work in this area (not yet done):** `CreateCompanyCommand`/`AddStaffMemberCommand` currently create a brand-new `User` by phone/email when the phone doesn't already exist — this doesn't match the corrected product model (should pick an *existing* registered user instead). Confirm scope with the user before reworking this; it's unscoped follow-up, not a defined task yet.
...
3. Task 11 was already tried and reverted — do not redo it. Read [[feedback-never-remove-registration-pointer]] first if anything about registration comes up.
